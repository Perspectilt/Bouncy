...
T, t, xvector, yvector, u_x, u_y, n, s0x, s0y = 0, 0, 0, 0, 0, 0, 1, 0, 0


# To add the laws of physics (very basic stuff for now -- limited to 1 object and 1 boundary only)
def physics(root, object, boundary, log=False):
    global t
    global xvector
    global yvector
    global u_x
    ...
    s_x, s_y = s0x - round(u_x * t + .5 * xvector/object[0].mass * t ** 2), s0y - round(u_y * t + .5 * yvector/object[0].mass * t ** 2)
    ...
    if not (object[0].pos_x <= boundary[0].pos()[0] or object[0].pos()[2] >= boundary[0].pos()[2]):
        removeforce(object[0], 'f_x')
    if not (object[0].pos()[1] <= boundary[0].pos()[1] or object[0].pos()[3] >= boundary[0].pos()[3]):
    ...
    if log:
        with open('debug.log', 'a') as f:
            f.write('\tobject[0]_at: ' + '(' + str(object[0].pos()[0]) + ',' + str(object[0].pos()[1]) + ')\n')

    # Changes v to u for the next iteration
    u_x, u_y = v_x, v_y

    # Collision physics (assumes completely elastic collision)  <!!!Fine-tuning needed!!!>
    if (v_x != 0) and (object[0].pos()[0] <= boundary[0].pos()[0] or object[0].pos()[2] >= boundary[0].pos()[2]):
        addforce(object[0], 'f_x', 2 * object[0].mass * u_x/(round(t), 1)[round(t) == 0], -math.copysign(1, object[0].mass * u_x/(round(t), 1)[round(t) == 0]) * 90)
        t = 1
        s0x, s0y = object[0].pos()[0], object[0].pos()[1]
        v_x, v_y = 0, 0
    if (v_y != 0) and (object[0].pos()[1] <= boundary[0].pos()[1] or object[0].pos()[3] >= boundary[0].pos()[3]):
        addforce(object[0], 'f_y', 2 * object[0].mass * u_y/(round(t), 1)[round(t) == 0], -math.copysign(1, object[0].mass * u_y/(round(t), 1)[round(t) == 0]) * 90)
        t = 1
        s0x, s0y = object[0].pos()[0], object[0].pos()[1]
        v_x, v_y = 0, 0


    t += .01   # Time counter (to be replaced in next version?)
    T += .01   # Master time-keeper
    ...
    # root.after(1, physics)        <--- Doesnt work yet :(
